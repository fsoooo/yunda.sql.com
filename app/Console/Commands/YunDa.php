...
class YunDa extends Command
{
     ...
     * @var string
     */
    protected $description = 'yunda Command description';
    ...
    {
//        //定时任务
//        $timed_task_res = TimedTask::where('task_name','yd_insure')->first();
//        if($timed_task_res->status=='0'){
        ...
        for($i=0;$i<$count;$i++) {
            ...
            foreach($value as $key=>$item){//每次1000条数据
                ...
                if($idCard_status['status']=='2') {
                    ...
                    if(json_decode($insure_status,true)['status']!=200){
...
//        }
    ...
    public function doInsurePrepare($prepare){
        ...
        $toubaoren['ty_toubaoren_id_type'] = isset($prepare['channel_user_type'])?$prepare['channel_user_type']:'1';//证件类型
        ...
        $beibaoren[0]['ty_beibaoren_id_type'] = isset($prepare['channel_user_type'])?$prepare['channel_user_type']:'1';
